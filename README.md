...
**Sovereign-IQ主权智感多智能体投研决策系统**

面向中国一级市场机构的投研决策系统 —— 7 个专业化 AI Agent 利用海量行业知识和专业技能，协同尽调、行研、对抗论证，萃取一级市场投资的真知灼见，最终产生投资决策建议，让长达数周的高门槛、专业化工作任务在2小时内完成。

本项目为NVIDIA DGX Spark Hackathon参赛作品，在单台DGX Spark上实现多模型并发，milvus向量数据库本地部署，纯本地化方案充分发挥设备性能，确保了一级市场投资项目资料的保密性和安全性，可定制化配置行研和投资策略、估值模型等参数，打造了符合国内一级市场机构的一站式“数智化合伙人团队”。
...
**极致性能双模驱动** —— 单机同时加载 Nemotron与 Qwen双模型，充分发挥nemotron模型的调度能力+千问模型中文金融语义理解和推理能力，结合 Embedding 与 Milvus 向量数据库（支持gpu加速），深度榨取硬件性能，实现“专模专用”的协同效应。

**智能体记忆优化** —— 利用milvus向量数据库，为每一位agent打造专业的背景知识库，让智能体拥有海量记忆、永久记忆、拟人记忆，让智能体能够基于项目底稿发表专业观点，并通过产生的知识经验再次导入milvus知识库，实现智能体持续进化。

**信息底稿全方位扫描**——秘书通过对底稿材料的深度学习，利用企查查数据接口及Tavily、EXA等搜索工具，全网获取项目信息，并进行比对，形成信息核对报告，发送给人类确认后，再开展投研工作。

**对抗式多轮论证** —— R1 独立分析 → R2 交叉质证（红蓝黄对抗） → R3 红蓝对抗，三轮递进消除信息盲区与群体思维，形成真知灼见。

**知识结晶与进化** —— 每次投委会讨论中产生的报告存入openclaw共享文件夹，所有agent可见，多轮辩论逐级萃取核心观点，有价值的报告经人类审核后，再次导入agent的知识库，实现agent的持续进化，同时，人类可以根据需要对agent专业背景知识库进行更新。

**全链路可审计** —— 从文档摄入到最终决策，完整记录推理链路、证据来源与观点演变过程。

**人机主权决策** —— AI 提供结构化分析框架与证据支撑，人类保留信息控制权和最终决策权，不替代判断。
